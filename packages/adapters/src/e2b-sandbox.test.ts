...
describe("E2B computer backend", () => {
  ...
  it.each([{ cpuCount: 4 }, { memoryMB: 8192 }])(
    "rejects partial managed-template resources: %o",
    async (resources) => {
      const sdk = {
        create: vi.fn().mockResolvedValue({ sandboxId: "profile-sandbox" }),
      } as unknown as E2BSandboxSdk;
      ...
      await expect(
        provider.provision({ botId: "bot-1", homePath: "/unused", ...resources }, context),
      ).rejects.toThrow("E2B CPU and memory must both be set or both be omitted");
      expect(templates.exists).not.toHaveBeenCalled();
      expect(templates.build).not.toHaveBeenCalled();
