...
export const ComputerProfileInputSchema = z
  ...
  .superRefine((input, context) => {
    if (input.kind === "e2b" && (input.cpuCount == null) !== (input.memoryMB == null)) {
      context.addIssue({
        code: "custom",
        path: [input.cpuCount == null ? "cpuCount" : "memoryMB"],
        message: "E2B CPU and memory must both be set or both be omitted",
      });
    }
